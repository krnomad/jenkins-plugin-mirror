...
**필수 도구:**
- **GitHub CLI (gh)**: GitHub Release 다운로드에 필요
  ```bash
  # Ubuntu/Debian
  sudo apt install gh
  ...
  ```
- **기본 도구**: `curl`, `sha256sum`, `tar`, `gunzip`
...
- **크기**: ~13GB (압축 전), 최대 50개 파트로 분할
- **플러그인**: 3,000+개 (최대한 완전한 미러)
- **특징**: 다양한 버전 지원, 레거시 호환성, 자동 체크섬 검증
- **업데이트**: 월 1회 자동 릴리즈 (multi-part 방식만 지원)
...
│   ├── split-and-upload.sh                # TAR 분할 방식 (압축 없음, 84GB 필요)
│   ├── split-and-upload-optimized.sh      # 최적화 방식 (60GB 필요)
│   ├── split-and-upload-streaming.sh      # 스트리밍 방식 (54GB 필요)
...
- **GitHub CLI (gh)**: 인증 설정 완료 (`gh auth login` 필요)
- **필수 도구**: `jq`, `rsync`, `wget`, `curl`, `git`, `sha256sum`, `tar`
...
#### 1. 기존 미러 활용 (권장)
```bash
# 기존 미러가 jenkins-comprehensive-mirror에 있는 경우
...
| 방식 | 스크립트 | 필요 용량 | 처리 방식 | Jenkins 플러그인 권장도 |
|------|----------|-----------|-----------|------------------------|
| **TAR 분할** | `split-and-upload.sh` | ~84GB | TAR 분할 (압축 없음) | ✅ |
| **최적화** | `split-and-upload-optimized.sh` | ~55GB | 청크 기반 분할 압축 | ⚠️ (재압축으로 용량 증가) |
| **스트리밍** | `split-and-upload-streaming.sh` | ~54GB | 진정한 스트리밍 압축 | ⚠️ (재압축으로 용량 증가) |

**Jenkins 플러그인용 TAR 분할 스크립트의 장점:**
- **재압축 없음**: 이미 압축된 플러그인 파일(.hpi/.jpi)을 다시 압축하지 않음
- **용량 증가 방지**: 압축된 파일을 재압축해서 80GB→160GB 되는 문제 해결
- **GitHub 호환**: 각 파트가 2GB 제한 준수
- **0으로 나누기 에러 수정**: 안전한 크기 계산 로직
- **안정성**: 실시간 체크섬 검증 및 자동 정리

**용량 시뮬레이션:**
```bash
# 42GB 미러 기준 디스크 사용량 비교
./upload_script/simulate-disk-usage.sh 42 30
```

**권장사항:**
- **Jenkins 플러그인**: `split-and-upload.sh` 사용 (압축 없음)
- **일반 파일**: `split-and-upload-streaming.sh` 사용 (압축 효과)
